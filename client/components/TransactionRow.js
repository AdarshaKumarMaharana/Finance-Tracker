export const createTransactionRow = (transaction) => {
  ...
  row.innerHTML = `
    <td class="px-6 py-4"><div class="font-semibold">${transaction.title}</div>
      <div class="text-xs text-gray-500">${transaction.category_name || 'Uncategorized'}</div></td>
    <td class="px-6 py-4 text-sm text-gray-500">${new Date(transaction.date).toLocaleDateString()}</td>
    <td class="px-6 py-4 text-right ${amountColor} font-medium">${amountSign} ₹${transaction.amount.toFixed(2)}</td>
