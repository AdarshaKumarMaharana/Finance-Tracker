...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  const tabTransactionContent = document.getElementById('tabTransactionContent');
  ...
  // --- Modal Functions ---
  const openModal = () => transactionModal && transactionModal.classList.remove('hidden');
  const closeModal = () => transactionModal && transactionModal.classList.add('hidden');
  const openConfirmModal = () => confirmModal && confirmModal.classList.remove('hidden');
  const closeConfirmModal = () => confirmModal && confirmModal.classList.add('hidden');
    ...
    row.innerHTML = `
      <td class="px-6 py-4">
        <div class="flex items-center gap-3">
          <span class="text-2xl">${transaction.icon || (transaction.type === 'income' ? '💰' : '🛒')}</span>
          <div>
            <div class="font-semibold text-base capitalize">${transaction.title}</div>
      ...
      <td class="px-6 py-4 text-sm text-gray-500 dark:text-gray-400">${formatDate(transaction.date)}</td>
...
const renderCategoryPieChart = () => {
  ...
  if (categories.length === 0) {
    container.innerHTML = '<div class="text-center text-gray-500 dark:text-gray-400 py-20 text-lg">No expense data yet</div>';
    return;
  }
...
const renderIncomeVsExpenseChart = () => {
  ...
  const hasData = months.some(m => monthly[m].income > 0 || monthly[m].expense > 0);
  ...
  const renderBudgetProgress = () => {
    ...
    const budgetedCategories = summaryData.categories.filter(cat => 
      cat.budget && Number(cat.budget) > 0
    );
...
<div class="flex items-start justify-between mb-5">
  <div class="flex items-center gap-4">
    ...
    <!-- Name + Spent Info -->
    <div>
      <h3 class="text-xl font-bold text-gray-900 dark:text-white capitalize">${cat.name}</h3>
      <p class="text-sm text-gray-500 dark:text-gray-400 mt-1">
        Spent ₹${Number(cat.spent || 0).toFixed(2)} of ₹${Number(cat.budget || 0).toFixed(2)}
      </p>
    </div>
  </div>
  ...
  <div class="text-right">
    <span class="text-2xl font-bold ${overBudget ? 'text-red-600' : 'text-green-600'}">
      ${progress.toFixed(0)}%
    </span>
    ${overBudget ? '<p class="text-xs text-red-600 font-medium mt-1">Over budget!</p>' : ''}
  </div>
</div>
...
<p class="text-xs text-gray-500 dark:text-gray-400 mt-3">
  ${overBudget ? 'Over budget! Consider reviewing your expenses.' : 'Stay on track!'}
</p>
`;
  ...
  const loadData = async () => {
    try {
      const [summaryRes, expensesRes, incomeRes, categoriesRes] = await Promise.all([
      ...
      totalBalanceEl.textContent = formatCurrency(summaryData.summary.savings || 0);
      totalIncomeEl.textContent = formatCurrency(summaryData.summary.totalIncome || 0);
      totalExpensesEl.textContent = formatCurrency(summaryData.summary.totalExpenses || 0);
...
openProfileOption.addEventListener('click', async (e) => {
  ...
  try {
    ...
    if (user.profileImageUrl) {
      ...
      document.getElementById('modalProfileFallback').classList.remove('hidden');
    ...
    // Total transactions
    document.getElementById('profileTransactions').textContent = transactions.length || 0;
...
const editAvatarPreviewWrapper = document.getElementById('editAvatarPreviewWrapper');
const editAvatarPreview = document.getElementById('editAvatarPreview');
const editAvatarUploadLabel = document.getElementById('editAvatarUploadLabel');
...
[document.getElementById('editFirstName'), document.getElementById('editLastName')].forEach(input => {
...
editProfileForm.addEventListener('submit', async (e) => {
  ...
  if (!firstName || !lastName || !email) return showToast('Please fill all required fields', 'error');

setEditLoading(true);

  try {
    let profileImageUrl = null;
    if (editAvatarFile) profileImageUrl = await uploadProfileImage(editAvatarFile);

    const res = await apiFetch('/update-profile', {
      method: 'PUT',
      body: JSON.stringify({ firstname: firstName, lastname: lastName, email, password: password || null, profileImageUrl })
    });
...
closeProfileModal.addEventListener('click', () => {
  ...
  if (tabTransaction && tabCategory) {
    tabTransaction.addEventListener('click', () => {
      tabTransaction.classList.add('text-primary', 'border-b-4', 'border-primary');
      tabTransaction.classList.remove('text-gray-500', 'dark:text-gray-400');
      tabCategory.classList.remove('text-primary', 'border-b-4', 'border-primary');
    ...
    tabCategory.addEventListener('click', () => {
      ...
      tabTransaction.classList.remove('text-primary', 'border-b-4', 'border-primary');
  ...
  if (categoryForm) {
    categoryForm.addEventListener('submit', async (e) => {
      ...
      const monthlyBudget = setBudget ? parseFloat(document.getElementById('monthlyBudget').value) : null;

      const emojiBtn = document.querySelector('#tabCategoryContent button[onclick="openEmojiPickerForIcon(this)"]');
...
const openTransactionModal = (transaction = null) => {
  ...
  const transactionEmojiBtn = document.querySelector('#tabTransactionContent button[onclick="openEmojiPickerForIcon(this)"]');
  const categoryEmojiBtn = document.querySelector('#tabCategoryContent button[onclick="openEmojiPickerForIcon(this)"]');

  // Reset emoji buttons to smile-plus icon
  if (transactionEmojiBtn) transactionEmojiBtn.innerHTML = '<i data-lucide="smile-plus"></i>';
  if (categoryEmojiBtn) categoryEmojiBtn.innerHTML = '<i data-lucide="smile-plus"></i>';
  lucide.createIcons();
  ...
  transactionForm.addEventListener('submit', async (e) => {
    ...
    const emojiBtn = document.querySelector('#tabTransactionContent button[onclick="openEmojiPickerForIcon(this)"]');
    const icon = emojiBtn ? emojiBtn.textContent.trim() : '💰';
  ...
  document.addEventListener("click", (e) => {
    if (!picker.contains(e.target) && e.target.closest("button") !== activeIconButton) {
      picker.classList.add("hidden");
    }
  });
  ...
  transactionListEl.addEventListener('click', async (e) => {
    if (e.target.closest('.delete-btn')) {
      ...
      newConfirmBtn.addEventListener('click', async () => {
        const endpoint = type === 'income' ? `/income/${transaction.id}` : `/expenses/${transaction.id}`;
        try {
  ...
  const categoryTypeRadios = document.querySelectorAll('input[name="categoryType"]');

  const toggleBudgetField = () => {
    const isExpense = document.querySelector('input[name="categoryType"]:checked').value === 'expense';
  ...
  categoryTypeRadios.forEach(radio => radio.addEventListener('change', toggleBudgetField));
  ...
  // --- Event Listeners ---
  if (openModalBtn) openModalBtn.addEventListener('click', () => openTransactionModal(null));
  if (closeModalBtn) closeModalBtn.addEventListener('click', closeModal);
  if (closeModalBtn2) closeModalBtn2.addEventListener('click', closeModal);
  if (transactionModal) transactionModal.addEventListener('click', (e) => e.target === transactionModal && closeModal());
  if (cancelConfirmBtn) cancelConfirmBtn.addEventListener('click', closeConfirmModal);
  if (confirmModal) confirmModal.addEventListener('click', (e) => e.target === confirmModal && closeConfirmModal());
